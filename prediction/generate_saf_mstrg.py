...
with open(sys.argv[1], "r+") as gtf, open(sys.argv[2], "r+") as non_coding_list, open(sys.argv[4], "w") as saf, open(sys.argv[5], "w") as bed, open(sys.argv[6], "r") as original_gff:
	...
	def get_attribute_value(attributes, attribute_name):
	    for attribute in attributes:
	        if attribute.strip().startswith(attribute_name):
	            return attribute.strip().split(" ")[1].strip('";')
	    return None
	...
	for line in gtf:
		...
		if line[2]=="transcript":
			col9=line[8].split(";")
			class_code = get_attribute_value(col9, "class_code")
			#print class_code
			...
			if class_code == "=":
				...
				mstrg=col9[0].split(" ")[1][1:-1]
				if mstrg in non_coding_ids:
					gene_name = get_attribute_value(col9, "cmp_ref_gene")
